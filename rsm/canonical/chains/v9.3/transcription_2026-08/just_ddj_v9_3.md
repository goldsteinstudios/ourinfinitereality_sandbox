...
⽞ :: Paradox :: Pₙ. 牝 :: generative. ⽞牝 :: generative paradox :: P₀. The compound is compositional — the character for paradox plus the character for generative capacity computes the name of the generative paradox — so the assignment is motivated, not brute. The differentiator: every Pₙ has two faces (in-frame, the locus circulation organizes around; trans-frame, the origin a child organizes around); P₀, unframed, has no in-frame face — it is *purely* generative, and 牝 marks that purity (可以為天下⺟; ⽞牝之⾨是謂天地根: the mother of the whole tree). ⽞之⼜⽞ :: the species iterated — paradox at every recursion of the tree: the reduplication reads as the recursion itself. 眾妙之⾨ (Ch. 1) and ⽞牝之⾨ (Ch. 6): ⾨ at exactly the two places the structure has gates — the recursion’s and the ground’s.

The three generative forms, one ratio, three mechanisms, never conflated: (1) entailment-⽣ :: the ground, P₀ : O₁ (道⽣⼀ — 道 is not a mother; P₀ has no prior state); (2) parturition-⽣ :: Pₙ → Pₙ, O₍ₙ₊₁₎ (severance with a persisting parent); (3) 相⽣ :: mutual constitution within the frame. ⽣ ≠ 相⽣; entailment-⽣ ≠ parturition-⽣.

## The count: 道⽣⼀，⼀⽣⼆，⼆⽣三，三⽣萬物 [R — Ch. 42 not in Guodian]

Exposition-order, never event-order. ⼀ :: the first distinction; ⼆ :: the two modes (nothing added — the two came with the one); 三 :: the pair plus its bond — the text’s own gloss in the next line, 沖氣以為和: the blend, not a third mode and not a third axis. The three is Pₙ — the pair with its bond; the text’s exposition counts children from there, which is commentary on how it counts and not a structural siting, since generation has no address. 三⽣萬物 because only the pair-with-its-bond is able to be referred to. The chapter’s staging is absent from the oldest witness; read as a timeline it re-imports what the strips refuse.

## 天地不仁; 守中 [mixed strata — only the bellows lines are strip-attested (A23)]

天地 :: Gₙ; 天地不仁 :: no preferred position along the gradient [R]. Extended to the tree: no preferred frame — every trunk is a branch of its parent and every branch a trunk to its children, save a root (each obtained tree is rooted; O₁’s parent-side is P₀, an impossibility, not a frame; the ground is exempt by mechanism, not favored). The bellows (虛⽽不屈，動⽽ 愈出) :: the functional void, strip-attested [G]. 守中 :: Bₙ — the balance line, which reaches from Oₙ through Pₙ and which nothing occupies; descriptive, not prescriptive [R].

## 弗居 [G — slip A17; the strongest strip-attested match in the chain]

成⽽弗居: completes, yet structurally-cannot-dwell. 成 :: completion in the practical register (the completion is real); 弗居 :: the origin-denial (弗, structural impossibility — exactly not 不, contingent negation). The engine’s two-denial structure, in two characters, on a strip. 無為 :: aligned persistence; 為 :: effortful action under misalignment.

## 此兩者同出⽽異名 [R]

Same origin, different register — the slice identity is its sharpest reading: the gradient and the orbit are literally 同出 (one law) ⽽異名 (two slices, two names); 同謂之⽞ — the shared origin is viewed at Pₙ, where the slices coincide: ⽞’s own locus. Two names for one law is exactly what an indexed second reading is.

## The 法 regress [G — slip A23]

⼈法地，地法天，天法道，道法⾃然. 法 :: the child-parent structural relation — pattern-taking, not substance-transfer, which is what the cross-frame share is. 道法⾃然 :: the terminus of the regress of grounds — the model takes its pattern from its own self-sufficiency, because beneath an impossibility there is nothing to stand on. 域中有四⼤: the fourth term 王 not yet derived.
...
- 恆/常 — invariance-across-frames (恆, per the 亙 paleography) vs. invariance-within-frame (常).

- ⽣/成 — the child-origin obtaining vs. the child frame completing (⼤器曼成).
