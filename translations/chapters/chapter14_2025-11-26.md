...
### Connection to 可 (Orthogonal Transformation)

The three observation attempts (視/聽/搏) are attempts to **transform** the implicit into explicit—to apply the 可 operator (making explicit what was implicit).

The failures (不見/不聞/不得) document where transformation fails: at the boundary where there's no differentiation to transform.

**Note:** 可 and i both describe orthogonal transformation, but they are parallel notations, not equivalences.

### Connection to Perpendicularity

The 迎之/隨之 failures connect to the perpendicularity constraint:

At the observation boundary, there's no gradient curve to intersect perpendicularly. You can't find the "front" to approach because there's no ∇G to orient toward. You can't find the "back" to follow because there's no balance line to trail.
