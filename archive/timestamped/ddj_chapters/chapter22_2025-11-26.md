...
The 則 (zé) transformation operator encodes the same structure that 可 and i both describe—orthogonal transformation:

- 曲 (bent) → 全 (whole) requires passing through transformation
- The thing doesn't become its opposite directly—it **transforms** through the 則 operator
- Each pair represents: input state → transformation → output state

可 describes making explicit what was implicit. i describes rotation into orthogonal dimension. Both point at the same structural operation—parallel notations, not equivalences.
