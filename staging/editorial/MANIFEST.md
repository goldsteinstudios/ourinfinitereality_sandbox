...
## set-aside-craft/ (5) — set aside by Will, 2026-08-03; do NOT ingest

The machine-drafted technology-encoding cluster (NotebookLM v0.97x layer), set aside whole:
Will has not explored this material, so it does not ride into the editorial phase regardless of
per-document condition. Contents: `expansion-technology-index.md`,
`expansion-loom-axiom-identity.md`, `expansion-scythe-not-knife.md`,
`expansion-hamon-chapter76.md` (also retire-ruled, R23),
`expansion-forge-transformation-grammar-v0972.md`.

Standing inputs for whenever it is explored: `reports/technology_index_anachronism_check.md`
(5 clean / 3 rework / 2 failed; stratum-tag gap flagged) and the R22/R23 rulings. **Scope note:**
the pot cluster (session addendum, R-pot) is Will's own ruled work and is NOT set aside;
`expansion-helix-structure.md` is not part of the technology-encoding thesis and stays in the
flat set.
